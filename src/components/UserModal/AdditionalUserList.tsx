...
export default function AdditionalUserList({ }: Props) {
    ...
    // Xử lý phần hiển thị members khi AdditionalUserList lần đầu được mở
    useEffect(() => {
        if (arrProject[0]?.id !== undefined) {
            const action = getDetailProjectByIdAPI(arrProject[0]?.id)
            dispatch(action);
        }
    }, [arrProject])
            ...
            <Modal title="User List" open={visible} onCancel={handleCancel} width={800}>
                <div className="row">
                                    ...
                                    <List.Item key={item.userId}>
                                        ...
                                        <div>
                                            <button className='btn btn-primary' onClick={() => {
                                                const action = assignUserProjectInCreateProject(Number(projectId), item.userId)
                                                dispatch(action);
                                            }}>Add</button>
                                        </div>
                                    </List.Item>
                    ...
                    <div className="col-5">
                        <h2>Projects</h2>
                        <p>Project List</p>
                        <select className='form-control mb-3' value={projectId} onChange={(e) => {
                            let { value } = e.target;
                            setProjectID(Number(value));
                            const action = getDetailProjectByIdAPI(Number(value))
                            dispatch(action);
                        }}>
                                    ...
                                    <List.Item key={item.userId}>
                                        ...
                                        <div>
                                            <button className='btn btn-danger' onClick={() => {
                                                const action = removeUserProjectAPIInCreatePost(projectId, item.userId)
                                                dispatch(action);
                                            }}>Remove</button>
                                        </div>
                                    </List.Item>
